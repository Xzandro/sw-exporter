module.exports = {
  ...
  init(proxy, config) {
    ...
    proxy.on('HubUserLogin', () => {
      if (config.Config.Plugins[this.pluginName].enabled) { proxy.log({ type: 'info', source: 'plugin', name: this.pluginName, message: 'You just logged into the game.' }); }
    });

    // or all API commands with the 'apiCommand' event
    proxy.on('apiCommand', (req, resp) => {
      if (config.Config.Plugins[this.pluginName].enabled) { this.processEveryCommand(proxy, req, resp); }
    });
  },
